<?php
	...
	class Utilities
	{
		...
		*/
		...
		function dateDisplayFormatEst($date)
		{
	...
	function addingToCartList($proid,$sizeTy,$price,$max)
		{	
		for($i=0;$i < $max;$i++){
		/*	if($_SESSION['cart'][$i]["productId"] == "" || $_SESSION['cart'][$i]["productId"] == null){
				$_SESSION['cart'][$i]['productId'] = $proid;
				$_SESSION['cart'][$i]['productSize'] = $sizeTy;
				$_SESSION['cart'][$i]['productQty'] = 1;
				}*/
			if(($proid == $_SESSION['cart'][$i]["productId"]) and ($sizeTy == $_SESSION['cart'][$i]["productSize"])){
				
			   $_SESSION['cart'][$i]["productQty"]	= $_SESSION['cart'][$i]["productQty"] + 1;	
						
				}else{
					$_SESSION['cart'][$max]['productId'] = $proid;
					$_SESSION['cart'][$max]['productPrice'] = $price;
					$_SESSION['cart'][$max]['productSize'] = $sizeTy;
					$_SESSION['cart'][$max]['productQty'] = 1;
					}
